...
void Player::Tick(uint8_t input) {
    ...
    if (input & INPUT_A) {
        if (input & INPUT_LEFT) strafeDelta--;
        if (input & INPUT_RIGHT) strafeDelta++;
    } else {
        if (input & INPUT_LEFT) turnDelta -= TURN_SPEED * 2;
        if (input & INPUT_RIGHT) turnDelta += TURN_SPEED * 2;
    }
    ...
    if (angularVelocity < turnDelta) angularVelocity++;
    else if (angularVelocity > turnDelta) angularVelocity--;

    angle += angularVelocity >> 1;

    if (input & INPUT_UP) moveDelta++;
    if (input & INPUT_DOWN) moveDelta--;

    if (this == &Game::GetLocalPlayer()) {
        static int tiltTimer = 0;
        tiltTimer++;

        if (moveDelta && USE_ROTATE_BOB) targetTilt = (int8_t)(FixedSin(tiltTimer * 10) / 32);
        else targetTilt = 0;
        ...
        if (Renderer::camera.tilt < targetTilt) {
            ...
            if (Renderer::camera.tilt > targetTilt) Renderer::camera.tilt = targetTilt;
        ...
        if (Renderer::camera.bob < targetBob) {
            ...
            if (Renderer::camera.bob > targetBob) Renderer::camera.bob = targetBob;
    ...
    if (mana < maxMana && reloadTime == 0) mana += manaRechargeRate;
    ...
    switch (Map::GetCellSafe(cellX, cellY)) {
        case CellType::Potion:
            if (hp < maxHP) {
                if (hp + potionStrength > maxHP) hp = maxHP;
...
bool Player::CheckCollisions() {
    ...
    if (IsWorldColliding()) return true;

    Player& otherPlayer = (this == &Game::players[0] ? Game::players[1] : Game::players[0]);
    if (Game::IsOnlineEnabled()) {
        if (IsOverlappingPoint(otherPlayer.x, otherPlayer.y)) return true;
    }

    if (EnemyManager::GetOverlappingEnemy(*this)) return true;
