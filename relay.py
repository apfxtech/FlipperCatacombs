...
VERBOSE = False
MAX_CONNECTIONS = 2
IGNORED_PORTS = [ '/dev/ttyAMA0', '/dev/cu.NothingEara', '/dev/cu.Bluetooth-Incoming-Port' , '/dev/cu.debug-console']

activeSerialPorts = {}
activeSockets = []


print("Starting relay. Press Ctrl-C to stop")
	...
	while running:
		...
		if numActiveConnections() < MAX_CONNECTIONS:
			...
			for port in availablePorts:
				if not port.device in activeSerialPorts and not port.device in IGNORED_PORTS:
					try:
